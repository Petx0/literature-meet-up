...
- Assign a country based on the novel's own time period and setting, not on real-world geographic lookup. Place names that exist in multiple countries (e.g. "Richmond," "Hampshire," "Lambton") must be resolved using the novel's primary setting: a 19th-century English novel's "Richmond" is Richmond, Surrey — not Richmond, Virginia. When a place name is ambiguous between its English original and a namesake elsewhere (North America, Australia, etc.), prefer the country consistent with the novel's established setting. If you are genuinely unsure, leave `country` null rather than guessing. Never assign a real-world city to what the text implies is a fictional place (estates, villages, manors): use `location_type: "fictional"` instead.
